# -*- coding: utf-8 -*-
"""
US Stock Prediction History & Stats Collector

미국 종목의 예측 이력 수집 및 적중률 통계 갱신
- us_stock_prediction_history: 일별 등급 이력 기록
- us_stock_prediction_stats: 종목별 적중률 집계
...
async def batch_record_grades(conn: asyncpg.Connection, start_date: date, end_date: date) -> int:
    ...
    query = """
    INSERT INTO us_stock_prediction_history (symbol, grade_date, predicted_grade, grade_direction)
    SELECT
        symbol,
        date as grade_date,
...
async def batch_update_returns(conn: asyncpg.Connection) -> int:
    ...
    FROM us_stock_prediction_history h
    JOIN us_daily p1 ON h.symbol = p1.symbol AND h.grade_date = p1.date
    JOIN us_daily p2 ON h.symbol = p2.symbol AND p2.date = h.grade_date + INTERVAL '90 days'
    ...
    update_query = """
    UPDATE us_stock_prediction_history AS h
...
async def update_stats(conn: asyncpg.Connection) -> int:
    ...
    query = """
    INSERT INTO us_stock_prediction_stats (
    ...
    SELECT
        ...
        (SELECT predicted_grade FROM us_stock_prediction_history
         WHERE symbol = h.symbol ORDER BY grade_date DESC LIMIT 1) as latest_grade,
        (SELECT grade_date FROM us_stock_prediction_history
...
async def backfill_history(conn: asyncpg.Connection) -> Dict[str, int]:
    ...
    1. us_stock_grade에서 가장 오래된 날짜부터 시작
    ...
    if not date_range or not date_range['min_date']:
        logger.warning("No grade data found in us_stock_grade")
...
async def full_rebuild(conn: asyncpg.Connection) -> Dict[str, int]:
    ...
    # Step 1: 기존 데이터 삭제
    delete_history = await conn.execute("DELETE FROM us_stock_prediction_history")
    delete_stats = await conn.execute("DELETE FROM us_stock_prediction_stats")
...
async def collect_today(conn: asyncpg.Connection) -> Dict[str, int]:
    ...
    target_date = latest['latest_date']
    logger.info(f"Collecting latest data: {target_date}")

    # 최신 날짜 등급 기록
    grades_count = await batch_record_grades(conn, target_date, target_date)
...
async def main():
    print("=" * 60)
    print("US Stock Prediction Collector")
    print("=" * 60)
    print()
    print("실행할 작업을 선택하세요:")
    print("  1. 과거 데이터 일괄 수집 (Backfill)")
    print("  2. 최신 데이터 수집 (자동 날짜 감지)")
    ...
    try:
        ...
        logger.info("US Prediction Collector completed successfully")
