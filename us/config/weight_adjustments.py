...
Usage:
    from us.config.weight_adjustments import (
...
File: us/config/weight_adjustments.py
Created: 2025-11-29
"""
...
ORS_RISK_LEVELS: Dict[str, Dict] = {
    'EXTREME_RISK': {
        ...
        'description': 'Extreme outlier risk - reduce position significantly'
    },
    'HIGH_RISK': {
        'min_score': 60,
        'max_score': 79,
        'position_multiplier': 0.50,  # 포지션 50% 축소
        'description': 'High outlier risk - reduce position moderately'
    },
    'MODERATE_RISK': {
        'min_score': 40,
        'max_score': 59,
        'position_multiplier': 0.75,  # 포지션 25% 축소
        'description': 'Moderate outlier risk - slight position reduction'
    },
    'NORMAL': {
        'min_score': 0,
        'max_score': 39,
        'position_multiplier': 1.0,   # 포지션 유지
        'description': 'Normal risk profile - no adjustment needed'
    },
}
...
def get_risk_level(ors_score: float) -> str:
    ...
    Returns:
        Risk level string (EXTREME_RISK, HIGH_RISK, MODERATE_RISK, NORMAL)
    """
    if ors_score >= 80:
        return 'EXTREME_RISK'
    elif ors_score >= 60:
