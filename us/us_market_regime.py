"""
US Market Regime Detector v3.0 (Phase 3.5 - HMM Integration)

시장 레짐 감지 및 동적 팩터 가중치 결정
+ 4-Dimension Regime Composite Score
+ HMM 기반 레짐 감지 (v2.0 학술 연구 기반)
+ 레짐별 정적 가중치 (Factor Timing 최소화)

Regimes (5-Regime Classification - Legacy):
...
Regimes (3-Regime Classification - HMM v2.0):
...
- Phase 3.4.2: us_move_index, us_dollar_index, us_credit_spread, us_fed_rrp, us_vix
...
# ========================================================================
# v2.0 HMM Regime Weights (3-State: BULL/NEUTRAL/BEAR)
# ========================================================================
# 학술 연구 기반 정적 가중치
...
class HMMRegimeDetector:
    ...
    학술 연구 근거:
    - MDPI JRFM 2020: HMM이 규칙 기반보다 높은 Sharpe ratio
    ...
    async def predict_current_regime(
        ...
        # v2.0 정적 가중치
        weights = REGIME_WEIGHTS_V2.get(current_regime, REGIME_WEIGHTS_V2['NEUTRAL']).copy()
        # description 제거
    ...
    def _default_response(self) -> Dict:
        """기본 응답 (NEUTRAL)"""
        weights = REGIME_WEIGHTS_V2['NEUTRAL'].copy()
        weights = {k: v for k, v in weights.items() if k != 'description'}
        return {
            'regime': 'NEUTRAL',
            'regime_id': -1,
...
class USMarketRegimeDetector:
    """시장 레짐 감지 및 동적 가중치 결정 (v3.0: HMM 지원)"""
    ...
    def __init__(
        ...
        Args:
            db_manager: AsyncDatabaseManager instance
            analysis_date: 분석 날짜 (None이면 최신)
            use_hmm: HMM 기반 레짐 감지 사용 여부 (v2.0)
            use_static_weights: 정적 가중치 사용 여부 (동적 Macro 조정 비활성화)
        """
        self.db = db_manager
        ...
        if use_hmm and not HMM_AVAILABLE:
            logger.warning("HMM requested but hmmlearn not installed. Falling back to rule-based.")
    ...
    async def detect_regime(self) -> Dict:
        """
        시장 레짐 감지 및 결과 반환 (Phase 3.5: HMM 지원)
        ...
        try:
            ...
            # 4. Yield Curve 조회 (Phase 3.4 Macro Engine)
            yield_data = await self._get_yield_curve()

            # 5. Phase 3.4.2: 추가 지표 조회
            extended_data = await self._get_extended_indicators()

            # 6. GDP/PMI 데이터 조회 (5-Macro Environment용)
            ...
            if self.use_static_weights:
                ...
                # 동적 가중치 (Macro 조정 포함)
                weights = self._get_weights_with_macro_adjustment(regime)

            # 10. Phase 3.4.2: 4-Dimension Composite Score 계산
            composite_scores = self._calculate_composite_scores()

            # 11. 5-Macro Environment 분류 및 3-Scenario 확률
            macro_env = self.detect_macro_environment()
            scenario_prob = self.get_scenario_probability(macro_env)

            logger.info(f"Market Regime: {regime} ({REGIME_WEIGHTS[regime]['description']}) [RULE_BASED]")
            logger.info(f"Macro Environment: {macro_env} ({MACRO_ENVIRONMENT[macro_env]['description']})")
            logger.info(f"Scenario Probability: 강세={scenario_prob['강세']:.0%}, "
                       f"횡보={scenario_prob['횡보']:.0%}, 약세={scenario_prob['약세']:.0%}")
            logger.info(f"Weights: V={weights['value']:.0%}, Q={weights['quality']:.0%}, "
                       f"M={weights['momentum']:.0%}, G={weights['growth']:.0%}")
            logger.info(f"Composite: L={composite_scores['liquidity_score']:.0f}, "
                       f"M={composite_scores['macro_score']:.0f}, "
                       f"V={composite_scores['volatility_score']:.0f}, "
                       f"R={composite_scores['risk_appetite_score']:.0f} "
                       f"-> Total={composite_scores['composite_score']:.0f}")
    ...
    async def _detect_regime_hmm(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"Market Regime: {regime} ({REGIME_WEIGHTS_V2[regime]['description']}) [HMM]")
            ...
            logger.error(f"HMM regime detection failed: {e}, falling back to rule-based")
            # HMM 실패 시 규칙 기반으로 fallback
            self.use_hmm = False
    ...
    def _get_weights_with_macro_adjustment(self, regime: str) -> Dict[str, float]:
        """
        레짐별 가중치 + Macro 조정 (Phase 3.4)

        Adjustments applied:
        1. Base regime weights
        2. Rate Sensitivity Matrix (Fed Rate + CPI 조합)
        3. Yield Curve Signal adjustment
        4. Real Rate adjustment
        """
        # 1. Base weights from regime
        base_weights = self._get_weights(regime)
        weights = base_weights.copy()

        # 2. Get macro indicators
        fed_rate = self.indicators.get('fed_rate', 5.0)
        cpi_yoy = self.indicators.get('cpi_yoy', 3.0)
        yield_signal = self.indicators.get('yield_curve_signal', 'NORMAL')

        # ========================================
        # Rate Sensitivity Matrix Adjustment
        # ========================================
        FED_THRESHOLD = 4.5
        CPI_THRESHOLD = 3.0

        high_rate = fed_rate > FED_THRESHOLD
        high_inflation = cpi_yoy > CPI_THRESHOLD

        if high_rate and high_inflation:
            # HIGH_RATE_HIGH_INFLATION: Defensive - Quality/Value up
            weights['value'] += 0.05
            weights['quality'] += 0.05
            weights['momentum'] -= 0.05
            weights['growth'] -= 0.05
            macro_env = 'HIGH_RATE_HIGH_INFLATION'
        elif high_rate and not high_inflation:
            # HIGH_RATE_LOW_INFLATION: Stable - moderate Quality bias
            weights['value'] += 0.03
            weights['quality'] += 0.02
            weights['growth'] -= 0.05
            macro_env = 'HIGH_RATE_LOW_INFLATION'
        elif not high_rate and high_inflation:
            # LOW_RATE_HIGH_INFLATION: Value preferred
            weights['value'] += 0.05
            weights['momentum'] -= 0.02
            weights['growth'] -= 0.03
            macro_env = 'LOW_RATE_HIGH_INFLATION'
        else:
            # LOW_RATE_LOW_INFLATION: Growth friendly
            weights['value'] -= 0.05
            weights['quality'] -= 0.02
            weights['momentum'] += 0.03
            weights['growth'] += 0.04
            macro_env = 'LOW_RATE_LOW_INFLATION'

        # ========================================
        # Yield Curve Signal Adjustment
        # ========================================
        if yield_signal == 'INVERTED':
            # Recession signal - defensive
            weights['value'] += 0.02
            weights['quality'] += 0.03
            weights['momentum'] -= 0.03
            weights['growth'] -= 0.02
        elif yield_signal == 'STEEPENING':
            # Expansion signal - aggressive
            weights['value'] -= 0.02
            weights['quality'] -= 0.02
            weights['momentum'] += 0.02
            weights['growth'] += 0.02
        ...
        if real_rate > 2.0:
            # High real rate: Value favorable
            weights['value'] += 0.02
            weights['growth'] -= 0.02
        elif real_rate < 0.0:
            # Negative real rate: Growth favorable
            weights['value'] -= 0.02
            weights['growth'] += 0.02

        # ========================================
        # Normalize weights (clamp + sum=1.0)
        # ========================================
        MIN_WEIGHT = 0.10
        MAX_WEIGHT = 0.50
        ...
        logger.info(f"Macro Adjustment: {macro_env}, Yield={yield_signal}, RealRate={real_rate:.1f}%")
